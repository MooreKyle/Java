...
public class moorePayApp {
	...
	System.out.printf(
	    ...
	    "\nGross Pay: $%.2f" + (grossPay = hoursWorked * hourlyPayRate) +
	    "\nDeductions:\n   Federal Witholding (%.2f%%): $%.2f" +
	    (federalWithholdRate = grossPay * federalTaxRate) +
	    "\n   State Witholding (%.2f%%): $%.2f" + (stateWithholdRate = grossPay * stateTaxRate) +
            "\n   Total Deduction: $%.2f" + (totalDeduction = federalWithholdRate + stateWithholdRate) +
	    "\nNet Pay: $%.2f" + (netPay = grossPay - totalDeduction)
	);
    }
}